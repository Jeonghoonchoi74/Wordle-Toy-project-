...
# 터미널 출력을 위한 ANSI 색상 코드 (GUI에서는 사용하지 않지만 호환성을 위해 유지)
class Colors:
    GREEN = '\033[92m'  # 정확한 글자, 정확한 위치
    YELLOW = '\033[93m' # 정확한 글자, 틀린 위치
    RESET = '\033[0m'   # 색상 초기화
    GRAY = '\033[90m'   # 단어에 포함되지 않는 글자

# GUI 색상 상수
GUI_COLORS = {
    'GREEN': '#6aaa64',
    'YELLOW': '#c9b458',
    'GRAY': '#000000',      # 답이 아닌 경우 검정색
    'WHITE': '#000000',     # 기본 텍스트 (흰 바탕이므로 검정 글씨)
    'BLACK': '#ffffff',     # 배경 (흰색)
    'FILLED': '#878a8c',
    'KEY_DEFAULT': '#d3d6da', # 키보드 기본 색상 (연한 회색)
    'TEXT_ON_COLOR': '#ffffff' # 색상 박스 위의 글씨는 흰색 유지
}

class WordManager:
    """
    단어 목록과 정답 단어 선택을 관리하는 클래스입니다.
    """
    def __init__(self):
        # 5글자 영어 단어 샘플 목록입니다.
        # 실제 애플리케이션에서는 이 목록을 파일에서 불러올 수도 있습니다.
        self.words = [
            # A
    ...
    def get_random_word(self):
        """목록에서 무작위로 5글자 단어를 반환합니다."""
        return random.choice(self.words).upper()

    def is_valid_word(self, word):
        """단어가 유효한지 확인합니다."""
        return len(word) == 5 and word.isalpha() and word in self.words

class WordleLogic:
    """
    게임의 핵심 로직을 담당합니다 (GUI와 분리).
    """
    ...
    def start_new_game(self):
        self.secret_word = self.word_manager.get_random_word()
        self.current_attempt = 0
        self.is_game_over = False
        print(f"Debug: Secret Word is {self.secret_word}") # 디버깅용 (실제 게임에서는 제거 가능)

    def check_guess(self, guess):
        """
        추측 단어를 평가하고 결과를 반환합니다.
        Returns: (feedback_colors, result_type)
        result_type: 'WIN', 'LOSS', 'CONTINUE'
        """
        feedback = [""] * 5
        ...
        for i in range(5):
            if guess_list[i] is not None:
                if guess_list[i] in secret_list:
                    feedback[i] = GUI_COLORS['YELLOW']
                    secret_list[secret_list.index(guess_list[i])] = None
                else:
                    feedback[i] = GUI_COLORS['GRAY']
...
class WordleGUI:
    """
    Tkinter를 이용한 Wordle GUI 클래스입니다.
    """
    def __init__(self, root):
        self.root = root
        self.root.title("Python Wordle")
        self.root.configure(bg=GUI_COLORS['BLACK'])
        self.root.geometry("400x750") # 키보드 영역을 위해 높이 증가

        self.word_manager = WordManager()
        self.logic = WordleLogic(self.word_manager)

        self.frames = []   # 6 rows
        self.labels = []   # 6x5 grid of labels
        self.key_labels = {} # 가상 키보드 레이블 저장용 (문자 -> Label 위젯)
        self.current_guess = ""
    ...
    def _setup_ui(self):
        # 상단 타이틀
        title_label = tk.Label(self.root, text="WORDLE", font=("Helvetica", 32, "bold"), 
                               bg=GUI_COLORS['BLACK'], fg=GUI_COLORS['WHITE'])
        title_label.pack(pady=(20, 10))

        # 게임 그리드 (6행 5열)
        grid_frame = tk.Frame(self.root, bg=GUI_COLORS['BLACK'])
        grid_frame.pack(pady=10)

        for row in range(6):
            row_frame = tk.Frame(grid_frame, bg=GUI_COLORS['BLACK'])
            row_frame.pack(pady=2)
            self.frames.append(row_frame)
            row_labels = []
            for col in range(5):
                label = tk.Label(row_frame, text="", width=4, height=2, 
    ...
    def _setup_keyboard(self):
        # 가상 키보드 프레임
        kb_frame = tk.Frame(self.root, bg=GUI_COLORS['BLACK'])
        ...
        for row_keys in keys:
            row_frame = tk.Frame(kb_frame, bg=GUI_COLORS['BLACK'])
            row_frame.pack(pady=2)
            for char in row_keys:
                # 각 키는 Label로 표시
                label = tk.Label(row_frame, text=char, width=4, height=2,
                                 font=("Helvetica", 10, "bold"),
                                 bg=GUI_COLORS['KEY_DEFAULT'], fg=GUI_COLORS['WHITE'],
                                 relief="flat")
                label.pack(side="left", padx=2)
                self.key_labels[char] = label
    ...
    def _reset_grid(self):
        for row in range(6):
            for col in range(5):
                self.labels[row][col].config(text="", bg=GUI_COLORS['BLACK'], fg=GUI_COLORS['WHITE'], relief="solid")

    def _reset_keyboard(self):
        for char, label in self.key_labels.items():
            label.config(bg=GUI_COLORS['KEY_DEFAULT'], fg=GUI_COLORS['WHITE'])
    ...
    def _handle_keypress(self, event):
        ...
        if len(key) == 1 and key.isalpha():
            ...
            if len(self.current_guess) == 5:
                ...
                self._show_temp_message("단어가 너무 짧습니다")

    def _update_current_row(self):
        row_idx = self.logic.current_attempt
        for col in range(5):
            char = self.current_guess[col] if col < len(self.current_guess) else ""
            self.labels[row_idx][col].config(text=char)
            # 입력 중인 칸 시각적 효과
            if char:
               self.labels[row_idx][col].config(relief="solid", borderwidth=2, fg=GUI_COLORS['WHITE']) 

    def _submit_guess(self):
        if not self.word_manager.is_valid_word(self.current_guess):
            self._show_temp_message("단어 목록에 없습니다")
            return

        feedback, result = self.logic.check_guess(self.current_guess)
        self._animate_feedback(self.logic.current_attempt - 1, feedback)
        self._update_keyboard_colors(self.current_guess, feedback)
        
        self.current_guess = ""

        if result == 'WIN':
            self.message_label.config(text=f"축하합니다! (재시작: 엔터)")
        elif result == 'LOSS':
            self.message_label.config(text=f"정답: {self.logic.secret_word} (재시작: 엔터)")

    def _animate_feedback(self, row_idx, feedback):
        # 간단한 색상 업데이트
        for col, color in enumerate(feedback):
            self.labels[row_idx][col].config(bg=color, fg=GUI_COLORS['TEXT_ON_COLOR'], relief="flat")

    def _update_keyboard_colors(self, guess, feedback):
        for char, color in zip(guess, feedback):
            if char in self.key_labels:
                lbl = self.key_labels[char]
                current_bg = lbl.cget("bg")
                
                # 색상 우선순위: GREEN > YELLOW > GRAY > KEY_DEFAULT
                if current_bg == GUI_COLORS['GREEN']:
                    continue # 이미 초록색이면 변경 안함
                if current_bg == GUI_COLORS['YELLOW'] and color != GUI_COLORS['GREEN']:
                    continue # 이미 노란색인데 초록색이 아니면 변경 안함
                
                lbl.config(bg=color, fg=GUI_COLORS['TEXT_ON_COLOR'])


    def _show_temp_message(self, msg):
        original_text = self.message_label.cget("text")
        self.message_label.config(text=msg)
        self.root.after(2000, lambda: self.message_label.config(text=original_text))
